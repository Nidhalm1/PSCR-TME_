...
void reader(FileQueue &fileQueue, ImageTaskQueue &imageQueue) {
  ...
  while (true) {
    ...
    QImage image = pr::loadImage(file);
    if (image.isNull()) {
      continue;
    }

    TaskData imageTask(image, file);
    imageQueue.push(imageTask);
  }
  std::stringstream ss;
...
void resizer(ImageTaskQueue &imageQueue, ImageTaskQueue &resizedQueue) {
  ...
  while (true) {
    ...
    QImage image = task.image;
    QImage resized = pr::resizeImage(image);
    TaskData resizedTask(resized, task.file); // pas de copie , une fausse copie car QT
...
void saver(ImageTaskQueue &resizedQueue, const std::filesystem::path &outputFolder) {
  ...
  while (true) {
    ...
    std::filesystem::path output = outputFolder / task.file.filename();
    pr::saveImage(task.image, output);
  }
  std::stringstream ss;
